<?php
    ...
    $query = <<<eventSQL
    SELECT e.date, e.category, e.start_time, e.end_time, e.fee, e.max_cap, a.name, a.image FROM event e INNER JOIN artist a ON a.code=e.artist_code WHERE e.date < '$currentDate';
    eventSQL;
    $query1 = <<<event1SQL
    SELECT e.date, e.category, e.start_time, e.end_time, e.fee, e.max_cap, a.name, a.image FROM event e INNER JOIN artist a ON a.code=e.artist_code WHERE e.date < '$currentDate' ORDER BY e.date;
    event1SQL;
    $query2 = <<<event2SQL
    SELECT e.date, e.category, e.start_time, e.end_time, e.fee, e.max_cap, a.name, a.image FROM event e INNER JOIN artist a ON a.code=e.artist_code WHERE e.date < '$currentDate' ORDER BY e.category;
    event2SQL;
    $query3 = <<<event3SQL
    SELECT e.date, e.category, e.start_time, e.end_time, e.fee, e.max_cap, a.name, a.image FROM event e INNER JOIN artist a ON a.code=e.artist_code WHERE e.date < '$currentDate' ORDER BY a.name;
    event3SQL;
